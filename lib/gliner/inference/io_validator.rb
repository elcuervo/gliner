...
module Gliner
  class Inference
    ...
    class IOValidator
      ...
      def self.validation_for_outputs(output_names, input_names)
      ...
      def self.validation_for_logits(input_names)
      ...
      def self.validation_for_span_logits(input_names)
      ...
      def self.ensure_expected_inputs!(expected_inputs, input_names)
        missing = expected_inputs - input_names
        raise Error, "Model missing inputs: #{missing.join(', ')}" unless missing.empty?
      end

      private_class_method :validation_for_outputs, :validation_for_logits, :validation_for_span_logits, :ensure_expected_inputs!
    end
  end
end
